...
export default function QuestionCard({ question }: { question: Question }) {
  ...
  useEffect(() => {
    setSaved(getSavedQuestions().has(question.id));
  }, [question.id]);
  ...
  return (
    <div
      className={`rounded-xl border transition-all duration-200 ${
        open ? "border-accent-blue/40" : "border-border hover:border-gray-600"
      } bg-bg-card`}
    >
      {/* Header */}
      <div
      ...
      >
        <div className="flex-1 min-w-0">
          <p className="text-sm font-semibold leading-6 text-white">{question.title}</p>
          <div className="mt-2 flex flex-wrap items-center gap-2">
            {/* Level badge — styled like helloInterview */}
            <span
              className={`rounded-full border px-2.5 py-0.5 text-xs font-semibold ${difficultyColor[question.difficulty]}`}
            >
              {difficultyLabel[question.difficulty]}
            </span>
            {/* Category */}
            <span className="rounded-full border border-border px-2.5 py-0.5 text-xs text-gray-500">
        ...
        <div className="flex items-center gap-2 shrink-0 mt-0.5">
          <button
            ...
            aria-label={saved ? "حذف از ذخیره‌شده‌ها" : "ذخیره سوال"}
          >
            {saved ? (
              <BookmarkCheck size={16} className="text-accent-yellow" />
            ) : (
              <Bookmark size={16} />
            )}
          </button>
